...
async def pve_confirmed(c: CallbackQuery, state: FSMContext):
    ...
    bot_msg = await c.message.answer(text="⚔️ Выбери действие:", reply_markup=BATTLE_MENU_Kb("🛡 Защищаться", "defence_mob"))
...
async def pve_attack(c: CallbackQuery, state: FSMContext, user: User):
    ...
    if enemy:
        ...
        await c.message.answer(text=f"🗡 \"{enemy.name}\" <b>♥:{enemy.health}</b>(-{(pre_health-enemy.health) if (pre_health-enemy.health)!=0 else 'miss'}) <b>|</b> "
                                    f"🛡:<b>{enemy.defence}</b>(-{(pre_defence-enemy.defence) if (pre_defence-enemy.defence)!=0 else 'miss'})")
        if enemy.health > 0:
            bot_msg = await c.message.answer(text="⚔️ Выбери действие:", reply_markup=BATTLE_MENU_Kb("🛡 Защищаться", "defence_mob"))
            await state.update_data({'bot_message_id': bot_msg.message_id})
                ...
                if hasattr(enemy, 'bonus'):
                    ...
                    try:
                        if user.xp + enemy.bonus >= total_xp:
                            ...
                            await c.message.answer(
                                text=f"🎊 Вы получили +{enemy.bonus}<i>XP</i>, в связи с чем ваш уровень повышен!\n"
                        ...
                        await user.update(balance=user.balance + round(enemy.bonus / 2)).apply()
                        await c.message.answer(text=f"💰 Вы получили +{round(enemy.bonus / 2)} <i>мoнет</i>!")
...
async def pve_defence(c: CallbackQuery, state: FSMContext, user: User):
    ...
    if enemy:
        ...
        await c.message.answer(text=f"⚔️ \"{c.from_user.first_name}\" <b>♥:{user.health}</b>(-{(pre_health-user.health) if (pre_health-user.health)!=0 else 'miss'}) <b>|</b> "
                                    f"🛡:<b>{user.defence}</b>(-{(pre_defence-user.defence) if (pre_defence-user.defence)!=0 else 'miss'})")
        # if user still alive:
        if user.health > 0:
            await user.update(health=user.health, defence=user.defence).apply()
            # next step:
            bot_msg = await c.message.answer(text="⚔️ Выбери действие:", reply_markup=BATTLE_MENU_Kb("🗡 Атаковать", "attack_mob"))
            await state.update_data({'bot_message_id': bot_msg.message_id})
        # if dead:
        else:
            try:
                if hasattr(enemy, 'bonus'):  # if not examinator (examinators have no bonuses):
                    await c.message.answer('============================')
                    await user.update(health=1, defence=0, xp=user.xp - enemy.bonus if user.xp - enemy.bonus > 0 else -1, 
                                      balance=(user.balance - enemy.bonus // 2 if user.balance - enemy.bonus // 2 > 0 else -1) if user.lvl>2 else user.balance).apply()
                    # encouraging user:
